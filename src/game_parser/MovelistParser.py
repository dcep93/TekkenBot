...
class MovelistParser:
    def __init__(self, movelist_bytes, movelist_pointer):
        ...
        self.names_double = movelist_bytes[header_length:unknown_regions[42]].split(b'\00')[4:]
        self.names = []
        for i in range(0, len(self.names_double) - 1, 2):
            self.names.append(self.names_double[i].decode('utf-8'))

        self.move_nodes_raw = movelist_bytes[unknown_regions[54]:unknown_regions[58]] #there's two regions of move nodes, first one might be blocks????
        self.move_nodes = []
        for i in range(0, len(self.move_nodes_raw), 40):
            self.move_nodes.append(MoveNode(self.move_nodes_raw[i:i+40], movelist_pointer, movelist_bytes, self.names))
    ...
    def header_line(self, line, movelist_bytes, movelist_pointer):
